...
class Gemma4Config(VerifyAndUpdateConfig):
    ...
    def verify_and_update_config(vllm_config: "VllmConfig") -> None:
        ...
        Gemma4 uses different head dimensions for sliding window (head_dim=256)
        vs full attention (global_head_dim=512) layers. vLLM's AttentionGroup
        system naturally handles this: layers with different head_sizes get
        separate groups and can use different backends (e.g., FlashAttn for
        head_dim=256, TritonAttn for head_dim=512 where FA is unsupported).

        Previously this forced TRITON_ATTN globally to avoid mixed-backend
        issues, but vLLM v1's per-group metadata builders and KV cache specs
        make mixed backends safe when both backends return None from
        get_required_kv_cache_layout() (FlashAttn and TritonAttn both do).
        """
        ...
        if (
            ...
            and max(head_dim or 0, global_head_dim or 0) > 256
        ):
            logger.info(
                "Gemma4 model has heterogeneous head dimensions "
                "(head_dim=%d, global_head_dim=%d). Allowing per-layer "
                "backend selection (FlashAttn for sliding, TritonAttn "
                "for full attention).",
                head_dim,
                global_head_dim,
            )
